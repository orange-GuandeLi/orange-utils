  ...
  return (
    <div className="h-full flex flex-col bg-background text-foreground">
      ...
      <header className="h-14 border-b border-separator flex items-center px-5 gap-2 shrink-0">
        <h1 className="text-sm font-semibold">API Request</h1>
          ...
          <Tabs className="flex-1 min-h-0" defaultSelectedKey="body">
            <Tabs.ListContainer>
              <Tabs.List aria-label="请求配置">
                <Tabs.Tab id="body" className="text-xs">Body<Tabs.Indicator /></Tabs.Tab>
            ...
            <Tabs.Panel id="headers" className="p-3 space-y-2 overflow-y-auto mt-0">
              {headers.map((h, i) => (
                <div key={i} className="flex items-center gap-2">
                  <Input className="flex-1" placeholder="Key" value={h.key} onChange={(e) => updateHeader(i, "key", (e.target as HTMLInputElement).value)} />
                  <Input className="flex-1" placeholder="Value" value={h.value} onChange={(e) => updateHeader(i, "value", (e.target as HTMLInputElement).value)} />
                  <Tooltip delay={0}>
              ...
              <Button size="sm" variant="ghost" onPress={addHeader}><Plus size={12} /><span className="text-xs">添加 Header</span></Button>
            </Tabs.Panel>
          </Tabs>
